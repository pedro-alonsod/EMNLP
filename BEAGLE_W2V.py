...
for epochsRITrain in range(11):
    for run in range(6):
        ...
        # print(envVecs_static)
        print('In context make')
        # print('range', range(5))
        text_file = open(data_file_name, "r")
        for line in text_file: #read line in the file
            words = line.split() # extract words from the line
            for i in range(len(words)):  # for each word in the words list
                # print('i', i, 'len(words)', len(words))
                for cA in range(len(words)):
                    if words[i] == words[cA]:
                        # print('n - 1 words:', len(words), 'jump')
                        continue
                    else:
                        contextVecs_cooccurrence[words[i]] += envVecs_static[words[cA]]
                # if (i-1) < 0 and (i+1) >= len(words):
                #     # we dont have any context just 1 word
                ...
                #     # contextVecs_cooccurrence[words[i]] += envVecs_static[words[i]]
                #     # while contextAll > -1:
                #     # contextVecs_cooccurrence[words[i]] +=  envVecs_static[words[i]] + envVecs_static[words[i+1]]
                #         # contextAll -= 1
                #     for cA in range(len(words)):
                #         contextAll[i] += words[cA]
                # # else:
                ...
                #     # can do everything so add everything
                #     # print('Else all here')
                #     # for contextAll in range(len(words)):
                #         # contextVecs_cooccurrence[words[i]] += envVecs_static[words[i]] + envVecs_static[words[i+1]] + envVecs_static[words[i-1]]
                #     contextVecs_cooccurrence[words[i]] += envVecs_static[words[contextAll]]
                # # contextVecs_cooccurrence
        text_file.close()

    #Normalize vectors before storing them
        for word in contextVecs_cooccurrence.keys():
            # print(word)
            temp=np.array(contextVecs_cooccurrence[word], dtype=float)
            ...
            contextVecs_cooccurrence[word]=temp
            # print('**************************Normalising **********************************')
            # maybe this time i dnt need to normalze it here the eval is doing thats
        # print(word_space, 'embeddings to test')

        # Save the context vecs
        fileSave = codecs.open('./contextVecs.csv', 'w+')
        for word, vec in contextVecs_cooccurrence.items():
        ...
        print('saved context into file: ./contextVecs.csv')
        
        dfContext = pd.DataFrame.from_dict(contextVecs_cooccurrence, orient='index')
        ...
        setOfWordNetVocab = set(contextVecs_cooccurrence.keys()) #context not WN!!!!!!!!!!!!!!!!!!!
        ...
        ############################## for this dict is over go to next this is context #########################

        # # Test BEAGLE context
        # wordXWordDict = dfContext
        ...
        # print(str(dimension) + " Percentage of correct answers with BEAGLE context: " + str(100 * right_answers / (number_of_tests - number_skipped_tests)) + f"% skipped tests {number_skipped_tests}")
        # dictToefl['BEAGLEContext'].append((100 * right_answers / (number_of_tests - number_skipped_tests)))
    ...
    # These are femteGrams up ^^^^^^^^^^^^ #########################################
        ...
        # Save the context vecs
        fileSave = codecs.open('./orderVecs.csv', 'w+')
        for word, vec in orderVecs_position.items():
            # print("\t".join(word.split('\t')[:101])+'\n')
            # text = "\t".join(row[0].split('\t')[:101])+'\n'
            text = f'{word} '
            for i in range(0, len(vec)):
        ...
        print('saved order into file: ./orderVecs.csv')

        print('In composite make')
        ...
        print('saved composite into file: ./compositeVecs.csv')
        ...
        # Save the context vecs
        fileSave = codecs.open('./EnvVecs.csv', 'w+')
        for word, vec in envVecs_static.items():
            # print("\t".join(word.split('\t')[:101])+'\n')
            # text = "\t".join(row[0].split('\t')[:101])+'\n'
            text = f'{word} '
            for i in range(0, len(vec)):
        ...
        print('saved env into file: ./EnvVecs.csv')
        ...
        setOfGensimVocab = set(w2vObjectRI.wv.vocab)
        setOfWordNetVocab = set(envVecs_static.keys()) #context not WN!!!!!!!!!!!!!!!!!!!
        ...
        #     print('w2v:', embeddings_index[elem], type(embeddings_index[elem]))
        #     break
            if len(envVecs_static[elem]) != 300:
                print('here', elem) #cast it to the fire
            w2vObjectRI.wv[elem] = np.asarray(envVecs_static[elem], dtype=np.float32)
        print('Done!!!')
        ...
        # print(w2vObject.wv['the'], 'after before')
        w2vObject.train(sentences, total_examples=w2vObject.corpus_count, epochs=epochsRITrain)#w2vObject.iter)
        # print(w2vObject.wv['the'], 'after train')
        # sys.exit()
        w2vObject.wv.save_word2vec_format(f'./BEAGLE/GensimOneBAEGLEEnv{epochsRITrain}Run{run}.txt', binary=False) #encoding='utf-8' )
        # print('saved')

        ############################## for this dict is over go to next this is environmental #########################
        ...
        # print(w2vObject.wv['the'], 'after train')
        # sys.exit()
        w2vObject.wv.save_word2vec_format(f'./BEAGLE/GensimOneBAEGLEOrder{epochsRITrain}Run{run}.txt', binary=False) #encoding='utf-8' )
        # print('saved')
        
        ############################## for this dict is over go to next this is order #########################

        dfComposite = pd.DataFrame.from_dict(compositeVecs_CoPlusOr, orient='index')
        ...
        setOfGensimVocab = set(w2vObjectRI.wv.vocab)
        setOfWordNetVocab = set(compositeVecs_CoPlusOr.keys()) #context not WN!!!!!!!!!!!!!!!!!!!
        ...
        #     print('w2v:', embeddings_index[elem], type(embeddings_index[elem]))
        #     break
            if len(compositeVecs_CoPlusOr[elem]) != 300:
                print('here', elem) #cast it to the fire
            w2vObjectRI.wv[elem] = np.asarray(compositeVecs_CoPlusOr[elem], dtype=np.float32)
        print('Done!!!')
        ...
        # print(w2vObject.wv['the'], 'after train')
        # sys.exit()
        w2vObject.wv.save_word2vec_format(f'./BEAGLE/GensimOneBAEGLEComposite{epochsRITrain}Run{run}.txt', binary=False) #encoding='utf-8' )
        # print('saved')


        ############################## for this dict is over go to next this is composite #########################

    ################################# from here down just tests ^^ make them into word2vec
        # # Test BEAGLE env
        # wordXWordDict = dfEnv
        # #Testing of the embeddings on TOEFL RI without negative info
        # a = 0.0 # accuracy of the encodings    
        ...
        # dictToefl['BEAGLEEnv'].append((100 * right_answers / (number_of_tests - number_skipped_tests)))
        ...
        # text_file.close()
        # a += 100 * right_answers / (number_of_tests - number_skipped_tests)
        # print(str(dimension) + " Percentage of correct answers with BEAGLE composite: " + str(100 * right_answers / (number_of_tests - number_skipped_tests)) + f"% skipped tests {number_skipped_tests}")
        # # dictToefl['BEAGLEComposite'].append((100 * right_answers / (number_of_tests - number_skipped_tests)))
        # # print('Run ', k, 'finished')

# dfToeflResults = pd.DataFrame.from_dict(dictToefl, orient='index')
# dfToeflResults['Mean'] = dfToeflResults.mean(axis=1)
# print(dfToeflResults)